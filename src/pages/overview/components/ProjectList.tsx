            ...
            {props.projects.map((project: ProjectInterface, key: number) => {
                return (
                    <React.Fragment key={key}>
                        <ContextMenu model={contextMenuItems} ref={contextMenuRef} onShow={e => onItemContextMenuClickShow(e, key)} onHide={onItemContextMenuClickHide}></ContextMenu>
                        <div onClick={e => onProjectOpen(e, key)} onContextMenu={(e) => contextMenuRef.current.show(e)} className="flex flex-row dark:text-white text-sm hover:bg-indigo-800 hover:shadow-md cursor-pointer py-3">
